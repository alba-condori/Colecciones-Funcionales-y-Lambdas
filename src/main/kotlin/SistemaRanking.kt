...
class SistemaRanking {
    ...
    fun ordenarPorEficiencia(empleados: List<Empleado>): List<Empleado> {
        ...
        return empleados.sortedByDescending { it.proyectosCompletados / it.anosExperiencia }
    }

    fun ordenarPorPuntuacionCompuesta(empleados: List<Empleado>): List<Empleado> {
        return empleados.sortedWith( //El sortedWith se usa cuando se requieren varios criterios
            compareByDescending<Empleado> { (it.evaluacionDesempeno * 2) + (it.proyectosCompletados * 0.1) }
                .thenBy { it.nombre }  // Se usa el then by porque si empato, se va a ordenar por el nombre
        )
    }

    fun ordenarITPrimero(empleados: List<Empleado>): List<Empleado> {
        return empleados.sortedWith(
            compareByDescending<Empleado> { it.departamento == "IT" }
                .thenBy { it.salario }
        )
        /*return empleados
            .sortedBy { it.salario }
            .sortedByDescending { it.departamento == "IT" }*/
    }
    ...
    fun obtenerTopEmpleados(
        ...
        return empleados
            ...
            .take(limite) //"Toma" cierta cantidad
    }
}